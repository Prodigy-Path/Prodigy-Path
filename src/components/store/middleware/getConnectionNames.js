...
const convertIdToName = (store) => (next) => async (action) => {
  if (action.payload?.action === 'CONNECTION_NAMES') {
    ...
    let filterConnections = chatConnection.filter((item) => user._id === item.mentor || user._id === item.protege);

    const connectionNames = [];

    async function bigFunction(){
      const grabConnectionPromises = filterConnections.map(async (connection) => {
        let role;
        if(user.role === 'mentor'){
          role = 'protege'
        }
